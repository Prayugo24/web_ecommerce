<?php
    include "../../db.php";
	$kd=$_GET['id'];
	$qry=mysqli_query($db,"SELECT * FROM buku WHERE id_buku='$kd'");
        	...
        	<form action="p_edit_buk.php" name="modal_popup" enctype="multipart/form-data" method="POST">
        		<div class="form-group" style="padding-bottom: 10px;">
                    ...
                    <select class="form-control" name="kategori">
                        <?php
                            $idkat = $r['id_kategori'];
                            $qrykat = mysqli_query($db,"SELECT * from kategori where id_kategori = '$idkat'");
                            $kat = mysqli_fetch_array($qrykat);
                            $kategori = $kat['kategori'];
                            $q_allkat = mysqli_query($db,"SELECT * from kategori");
                        ?>
                            <option selected value=""><?php echo $kategori ?></option>
                            <?php while($kat1 = mysqli_fetch_array($q_allkat)){ ?>
                            <option value="<?php echo $kat1['kategori']; ?>"><?php echo $kat1['kategori']; ?></option>
                            <?php } ?>
                    </select>
                </div>
                <div class="form-group" style="padding-bottom: 10px;">
