...
if($set=="tanggal")
{
  ...
  $query_tanggal   = mysqli_query($db,"SELECT * from selesai where tgl_order like '$tanggal'");
}
else if($set=="status_kirim")
{
  $status_kirim = $_POST['status_kirim'];
  $query_status_kirim = mysqli_query($db,"SELECT * FROM selesai where status_pengiriman='$status_kirim'");
}
...
$no =1;
  ...
  $qrycus = mysqli_query($db,"SELECT * from customer where id_cus='$data[id_cus]'");
