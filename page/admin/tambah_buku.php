...
$id_kata = mysqli_query($db,"SELECT * from katalog where katalog='$katalog'");
$id_katalo = mysqli_fetch_array($id_kata);
$id_katalog = $id_katalo['id_katalog'];
@$message		= '';
$valid_file		= true;
$max_size		= 10240000; // Ukuran maksimal file yang akan diupload (dalam byte)
...
if($_FILES['gambar']['name']){
	...
	if(!$_FILES['gambar']['error']){
		...
		if($valid_file == true)
		{
			...
			// simpan ke database
			mysqli_query($db,"INSERT into buku set judul='$judul',id_kategori='$id_kategori',id_katalog='$id_katalog',pengarang='$pengarang',penerbit='$penerbit',hal='$halaman',harga='$harga',deskripsi='$deskripsi',tanggal='$tanggal',gambar='$nama_file_baru'");
