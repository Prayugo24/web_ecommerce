...
$kd = $_GET['id'];
session_start();
$q_detail = mysqli_query($db,"SELECT * from buku where id_buku='$kd'");
	...
	<div class="modal-content">
		...
		<div class="modal-footer">
            ...
            <input type="hidden" name="harga" value="<?php echo $detail['harga']; ?>"  style="width:10px;">
              <?php
              $qrystok = mysqli_query($db,"SELECT * FROM stok where id_buku='$detail[id_buku]'");
              ...
              ?>
                  Qty : <input type="number" name="qty" value="1" min="0" max="<?php echo $stok['stok']; ?>" style="width:70px;height:33px;">
	                <?php
                  $q_cek_s = mysqli_query($db,"SELECT * from selesai where id_cus='$_SESSION[id_cus]' && status_beli='lunas' && status_pengiriman='belum dikirim'");
                  $cek_s = mysqli_num_rows($q_cek_s);
                  $q_cek_z = mysqli_query($db,"SELECT * from selesai where id_cus='$_SESSION[id_cus]' && status_beli='lunas' && status_pengiriman='dikirim'");
                  $cek_z = mysqli_num_rows($q_cek_z);
                  $query_kode_beli = mysqli_query($db,"SELECT * from selesai where id_cus='$_SESSION[id_cus]' && status_beli='order' && status_pengiriman='belum dikirim'");
                  $data_kode_beli = mysqli_fetch_array($query_kode_beli);
                  $kode_beli = $data_kode_beli['kode_beli'];
                  $query_tujuan = mysqli_query($db,"SELECT * from tujuan where kode_beli='$kode_beli'");
                  $sql_tujuan = mysqli_num_rows($query_tujuan);
                  $query_selesai = mysqli_query($db,"SELECT * from  selesai where id_cus='$_SESSION[id_cus]' && status_beli='lunas' && status_pengiriman='belum dikirim'");
                  $sql_selesai = mysqli_num_rows($query_selesai);
                  if($sql_selesai>=1)
                  { ?>
                   <a href="home.php?pesan=status belum kirim" class="btn btn-success">Add to Cart</a>
                <?php  }
                  else if($sql_tujuan>=1)
                  { ?>
