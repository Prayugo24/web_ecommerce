...
{
	$idbuk = $_GET['idbuku'];
	$qry_stok = mysqli_query($db,"SELECT * from stok where id_buku='$idbuk'");
...
<div id="menu">
	...
	@$keranjong = mysqli_fetch_array(mysqli_query($db,"SELECT SUM(qty) as qty_total from keranjang where id_cus='$idcos' && kode_beli='$kode_beli'"));
	...
	<ul>
		...
		<?php
		$qryba = mysqli_query($db,"SELECT * from selesai where id_cus='$idcos' && status_beli='order' || status_beli='lunas'");
		$databa = mysqli_fetch_array($qryba);
		$tot_bbb = $databa['total_bayar'];
		$q_cek_k = mysqli_query($db,"SELECT * from selesai where id_cus='$idcos' && status_beli='order' || status_beli='lunas' && status_pengiriman='belum dikirim' || status_pengiriman='dikirim'");
...
<div id="content">
	<div id="contentkiri">
		...
		<?php if($hal==""){ ?>
			...
			if($id_kategori>=1 && $id_katalog>=1) { while($seleksi_buku1=mysqli_fetch_array($q_seleksi_buku1)){?>
			<div class="col-md-3">
			...
			<?php }}
			else if($id_katalog>=1) { while($seleksi_buku1=mysqli_fetch_array($q_seleksi_buku1)){?>
			<div class="col-md-3">
			<div class="tamp_produk" style="border:none;">
				<?php include("seleksi_produk1.php"); ?>
			</div>
			...
			if($aksi=="cari"){
			 $search= $_GET['search'];
			 $qry_cari_buku = mysqli_query($db,"SELECT * from buku where judul like '%$search%'");
